...
export default function GitHubConnectModal({
  ...
  async function submit() {
    ...
    try {
      ...
      if (!ok.fine_grained) {
        setWarning(
          "This is a classic personal access token (broad scopes). Consider creating a fine-grained PAT scoped only to the repos Kai needs to read."
        );
      }
      onConnected();
    } catch (err) {
        ...
        {status === "connected" ? (
          ...
          <div className="p-5 space-y-4">
            <div className="space-y-2 text-white/60 text-xs leading-relaxed" style={monoStyle}>
              <p>
                Paste a <strong className="text-white">GitHub fine-grained personal access token</strong> with read-only access.
              </p>
              <ul className="list-disc list-inside space-y-1 text-white/40">
                <li>Kai will only call read endpoints (repo contents, issues, PRs, commits, code search).</li>
                <li>We never push, merge, or modify anything on your repos.</li>
                <li>Your token is encrypted at rest and never sent to other services.</li>
                <li>Public repos work without a token at a 60-req/hr limit; connecting raises it to 5,000/hr.</li>
              </ul>
              <p className="text-white/40">
                ...
                . Grant <em>Contents: Read</em>, <em>Metadata: Read</em>, <em>Issues: Read</em>, and <em>Pull requests: Read</em>.
              </p>
            </div>
            ...
            <button
              ...
              style={monoStyle}
            >
              {status === "connecting" ? "Validating with GitHub…" : "Connect"}
            </button>
          </div>
        )}
      </div>
...
function humanizeError(raw: string): string {
  if (raw === "github_token_invalid") {
    return "GitHub rejected this token. Double-check it's not expired or revoked.";
  }
  if (raw === "github_token_forbidden") {
    return "Token authenticated but lacks permission. Make sure it's granted at least 'Metadata: Read'.";
  }
  if (raw === "github_validation_failed") {
    return "Couldn't verify this token with GitHub. Try again in a moment.";
  }
  if (raw === "invalid_token_format") {
    return "That doesn't look like a GitHub PAT. Tokens start with github_pat_ (fine-grained) or ghp_ (classic).";
  }
  if (raw === "token_required") return "Please paste a GitHub PAT.";
  if (raw === "store_failed") return "Couldn't save the token. Try again.";
  if (raw === "unauthorized") return "Sign in again before connecting GitHub.";
